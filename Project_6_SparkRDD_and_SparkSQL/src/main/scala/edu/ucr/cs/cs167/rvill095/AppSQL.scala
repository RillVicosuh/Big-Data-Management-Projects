    ...
    try {
      ...
      command match {
        ...
        case "code-filter" =>
          ...
          val query: String = s"SELECT COUNT(*) FROM log_lines WHERE response='$responseCode'"// TODO 10: write SQL query to filter view `log_lines` by `responseCode`
          val count: Long = spark.sql(query).first().getAs[Long](0)
        ...
        case "time-filter" =>
          ...
          val query: String = s"SELECT COUNT(*) FROM log_lines WHERE time BETWEEN $from AND $to"// TODO 11: write SQL query to filter view `log_lines` by `time` between `from` and `to`
          val count: Long = spark.sql(query).first().getAs[Long](0)
        ...
        case "count-by-code" =>
          ...
          println("Code,Count")
          val query: String = "SELECT response, COUNT(*) as count FROM log_lines GROUP BY response ORDER BY response ASC"// TODO 12: write SQL query the get the count for each `response`, results must be ordered by `response` in ascending order
        ...
        case "sum-bytes-by-code" =>
          ...
          val query: String = "SELECT response, SUM(bytes) as sum_bytes FROM log_lines GROUP BY response ORDER BY response ASC"// TODO 13: write SQL query the get the sum of `bytes` for each `response`, results must be ordered by `response` in ascending order
            spark.sql(query).foreach(row => println(s"${row.get(0)},${row.get(1)}"))
        case "avg-bytes-by-code" =>
          ...
          val query: String = "SELECT response, AVG(bytes) as avg_bytes FROM log_lines GROUP BY response ORDER BY response ASC"// TODO 14: write SQL query the get the average of `bytes` for each `response`, results must be ordered by `response` in ascending order
            spark.sql(query).foreach(row => println(s"${row.get(0)},${row.get(1)}"))
        case "top-host" =>
          ...
          val query: String = "SELECT host, COUNT(*) as count FROM log_lines GROUP BY host ORDER BY count DESC LIMIT 1"// TODO 15: write SQL query the get the `host` with the largest count
